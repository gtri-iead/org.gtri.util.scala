...
class ClassLog(val name : String, val qualifiedName : String) extends exelog.ClassLog {
  ...
  val logger = Logger.getLogger(qualifiedName)

  def isEnabledFor(level: Level) = logger.isEnabledFor(level)

  def log(fqcn: String, level: Level, message: => String, cause: Option[Throwable]) = logger.log(fqcn, level, message, cause.orNull)
}
object ClassLog {
